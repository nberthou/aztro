...
export const handleRedemptions = (pubSubClient: PubSubClient, chatClient: ChatClient) => {
  ...
  pubSubClient.onRedemption(channelId, async (msg) => {
    if (msg.rewardTitle.startsWith('Echanger')) {
      const currentUserInstance = new User(msg.userName.toLocaleLowerCase());
      const user = await currentUserInstance.getUser();
      if (!user) {
        await new User(msg.userName.toLocaleLowerCase()).createUser({ initialStars: msg.rewardCost });
      } else {
